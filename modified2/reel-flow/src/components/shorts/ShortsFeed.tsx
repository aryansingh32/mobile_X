...
export const ShortsFeed = React.memo(function ShortsFeed({ startVideoId, onVideoStarted, isFocused = true }: { startVideoId?: string | null; onVideoStarted?: () => void; isFocused?: boolean } = {}) {
  ...
  const renderItem = useCallback(({ item, index }: { item: ShortData; index: number }) => {
    ...
    // When the tab isn't focused, isActive/isPreload both go false above —
    // unmounting every WebView, including the one for the video the user
    // was actually watching. That's fine for the rest of the preload
    // window, but it meant returning to Shorts always cold-booted a brand
    // new YouTube iframe for the current video (DNS/TLS, iframe API script,
    // embedded player DOM all over again) even though nothing about it
    // needed to change. Keep just that one slot's WebView mounted — falls
    // through to ShortItem's existing "not active, not preloadAhead" branch
    // (muted + paused), so it costs nothing extra to implement and doesn't
    // reintroduce a multi-WebView background cost.
    const keepWarm = !isFocused && distance === 0;
